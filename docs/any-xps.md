...
## 5. 真实模型验证（2026-06-10，本机 Blender 3.6.21）

四个真实 XPS 模型端到端（转换 → 与各自参考 PMX 并排 → 同套 yaoxiang VMD → FK σ/IK 角差判定）：

| 模型 | 骨架家族 | 结果 |
|---|---|---|
| Purifier Inase（标定） | bip001 系 XNALara | **PASS** 20/20 步，FK σ 全 0，腿 2.9°（优于远端基线 4.4°） |
| Tifa Gantz | Daz 系（链内 Bend/Twist + c_thigh 矫正骨 + 跖骨链） | **PASS** FK σ 0，腿 5.2° |
| Reika | Daz 系 | **PASS** FK σ 0，腿 6.9° |
| rouffe | XNALara 改造（肩挂脖根、腿挂 spine lower、长外套链） | 识别/权重/腿全对；手臂 FK σ 3-8° 未达 σ<0.05（见遗留#1） |

真实模型暴露并已修复的缺陷（合成矩阵已补对应回归用例，现 19 用例）：

1. **4 节臂链被关节评分误判**（inase 短锁骨 + 弯角加分奖励锁骨拐点）→ 经典 4 节链改回位置映射，评分只用于 >4 节/3 节，去掉弯角项。
2. **props 跨模型污染**：识别失败的槽位保留上一模型残留 → identify 全量写入（含空值），打印不再回退 scene props。
3. **腿 fork 骨被映射为センター后权重被清**（rouffe 腿挂 spine lower=下背权重大户）→ fork 骨改映射 **下半身**（带权重变形骨），センター 由 complete 新建；fork 上方居中祖先 helper 归 pelvis→下半身；控制骨残留权重并入下半身。
4. **居中直通骨两侧提升**（tifa：hip→pelvis→双腿，pelvis 只进了 left 列表）→ 居中孩子按其孙辈所及的每一侧入候选。
5. **fork 扫描排除倒数第二节**（rouffe 肩挂 head neck lower）→ 放开到链尾前一节，新增「fork 之上仅剩头 ⇒ fork=首、上半身2=fork 下一节」语义。
6. **腿链「取前4」错位**（tifa c_thigh 矫正骨开头）→ 腿链关节分段（踝=首个 z<0.12H 节点、膝=高度中点、腿根=膝上最近的长度比合理节点）。
7. **Daz lMetatarsals 贴脚跟被当足先EX**（reika 足首 退化向量 119°）→ 趾骨须显著在脚踝前方（y < 踝-0.02H），否则沿链后找/由 complete 合成。
8. groups/ik 的模式健壮性（pose.group_add 需 POSE 模式；IK 提前返回须回 OBJECT）。

另：旧版「逐顶点最近」曾把腕部 helper 权重错倒进 親指０（拇指渗出 177 中约 157 是这种垃圾）；fold 后渗出降到 20、掌部 1059 与基线逐字一致、残留 4 顶点为斜坡设计内——净改进。

## 6. 遗留事项

1. **rouffe 手臂 FK σ 3-8°**：识别/层级/权重均正确（目标 PMX 也是标准 肩P/腕捩 结构），σ 来源未定位（疑与该参考模型的付与/轴设定有关）；渲染上动作大体跟住。后续可对比两模型 VMD 应用后的局部四元数定位。
2. 不同来源模型的**站距/比例差**（rouffe f150 腿开度大于参考）：IK 位置随转换模型自身 rest 比例，属「比例差≠姿态差」范畴，如需贴参考需做 IK 位置重定标（超出忠实转换范畴）。
